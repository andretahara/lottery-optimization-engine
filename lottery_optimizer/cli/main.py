...
def list_games() -> None:
    ...
    table.add_column("marcas")
    for slug in registry.available():
        s = registry.get(slug)
        table.add_row(slug, s.name, str(s.pool), str(s.draw_size), f"{s.min_marks}-{s.max_marks}")
    console.print(table)


@app.command("generate")
def generate(game: str, tickets: int = 10, marks: int = 0, seed: int = 12345) -> None:
    """Gera uma carteira (sera ligado quando os otimizadores forem implementados)."""
    console.print(f"[yellow]{DISCLAIMER}[/yellow]")
    raise typer.Exit(code=1)  # geracao real chega nos Blocos 2/5
